  ...
  return (
    <div className="p-8 space-y-8">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">Verify agent reliability, audits, and security certifications</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="border border-border rounded-lg p-4 bg-card">
          ...
          <p className="text-3xl font-bold">{auditDetails.totalAudits}</p>
          <p className="text-xs text-green-600 mt-1">{auditDetails.passRate}% pass rate</p>
        </div>
        <div className="border border-border rounded-lg p-4 bg-card">
          <p className="text-xs text-muted-foreground mb-1">Average Score</p>
          <p className="text-3xl font-bold">{auditDetails.avgScore}</p>
      ...
      <div className="border border-border rounded-lg bg-card p-6">
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {auditDetails.certifications.map((cert, idx) => (
            <div key={idx} className="border border-border rounded-lg p-4 flex items-center gap-3">
            ...
            <div key={agent.id} className="border border-border rounded-lg p-4 hover:border-accent transition">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{agent.decisions.toLocaleString()} decisions made</p>
                </div>
                <div className="text-right">
                  <p className="text-3xl font-bold">{agent.trustScore}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold">{agent.errorRate}%</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Audited</p>
                  <p className="text-sm font-semibold">{agent.audited ? "✓ Yes" : "✗ No"}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Insured</p>
                  <p className="text-sm font-semibold">{agent.insured ? "✓ Yes" : "✗ No"}</p>
                </div>
              </div>

              <div className="flex gap-2 flex-wrap mb-4">
                {agent.badges.map((badge, idx) => (
                  <span key={idx} className="text-xs bg-green-500/10 text-green-700 px-2 py-1 rounded">
                    {badge}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-2">
                <button className="flex-1 px-4 py-2 border border-border rounded-md hover:bg-secondary transition text-sm font-medium">
                  View Audit
                </button>
                {!agent.insured && (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[
            ...
            <div key={idx} className="border border-border rounded-lg p-4">
              ...
              <span className="text-xs bg-green-500/10 text-green-700 px-2 py-1 rounded">✓ {feature.status}</span>
            </div>
          ))}
        </div>
      </div>
